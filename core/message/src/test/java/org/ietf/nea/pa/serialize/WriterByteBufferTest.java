...
public class WriterByteBufferTest {
	...
	public void serializePaMessageWithMixedAttributes() throws ValidationException{
		...
		Assert.assertEquals(PaAttributeTypeEnum.IETF_PA_NUMERIC_VERSION.id(),  ByteArrayHelper.toLong(Arrays.copyOfRange(serialized, 29, 32)));
		Assert.assertEquals(258,  ByteArrayHelper.toLong((Arrays.copyOfRange(serialized, 45, 48))));
		
		Assert.assertEquals(PaAttributeTypeEnum.IETF_PA_INSTALLED_PACKAGES.id(),  ByteArrayHelper.toLong(Arrays.copyOfRange(serialized, 57, 60)));
