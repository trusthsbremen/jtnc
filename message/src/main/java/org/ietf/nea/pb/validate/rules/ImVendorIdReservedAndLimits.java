...
public class ImVendorIdReservedAndLimits {
	...
	public static void check(final long vendorId) throws ValidationException{
	        ...
	        if(vendorId > IETFConstants.IETF_MAX_VENDOR_ID){
	            throw new ValidationException("Subvendor ID is to large.",true,PbMessageErrorCodeEnum.IETF_INVALID_PARAMETER.code(),PbErrorCauseEnum.VALUE_TO_LARGE.number(),Long.toString(vendorId));
	        }
	        
	        if(vendorId < 0){
	            throw new ValidationException("Subvendor ID cannot be negativ.",true,PbMessageErrorCodeEnum.IETF_INVALID_PARAMETER.code(),PbErrorCauseEnum.NEGATIV_UNSIGNED.number(),Long.toString(vendorId));
	        }
