	...
	public static void check(final PbBatchDirectionalityEnum direction, final PbBatchTypeEnum type) throws ValidationException{
		...
		if(direction == PbBatchDirectionalityEnum.TO_PBS){
			if(type == PbBatchTypeEnum.RESULT || type == PbBatchTypeEnum.SDATA || type == PbBatchTypeEnum.SRETRY){
				throw new ValidationException("The batch type "+type.toString()+" is not expected in the direction "+direction.toString()+".", true,PbMessageErrorCodeEnum.IETF_UNEXPECTED_BATCH_TYPE.code(),PbErrorCauseEnum.BATCH_DIRECTION_OR_TYPE_UNEXPECTED.number(),type.toString(), direction.toString());
			}
