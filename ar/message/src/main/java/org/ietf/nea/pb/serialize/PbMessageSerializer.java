...
public class PbMessageSerializer implements
		TnccsSerializer<PbMessage> {

	private static final int MESSAGE_HEAD_SIZE = 
			PbMessage.FIXED_LENGTH; // in byte

	private Map<Long, Map<Long, TnccsSerializer<AbstractPbMessageValue>>> pbMessageTranslators;

	public PbMessageSerializer(){
		pbMessageTranslators = new HashMap<>();
	}
	...
	public PbMessage decode(final InputStream in, final long length)
		...
		PbMessageBuilderIetf pbBuilder = new PbMessageBuilderIetf();
		...
		if (this.pbMessageTranslators.containsKey(vendorId)) {
			if (this.pbMessageTranslators.get(vendorId)
				...
				pbBuilder.setFlags(flags.toArray(new PbMessageFlagsEnum[flags
						.size()]));
				...
				try{
					message = pbBuilder.toMessage();
				}catch (ValidationException e){
					throw new SerializationException("The message is not valid because of constraint violations.", e);
				}
				return message;
			}
		}
	...
	public void addSerializer(Long vendorId, Long messageType,
		...
		if(pbMessageTranslators.containsKey(vendorId)){
			...
			pbMessageTranslators.put(vendorId, new HashMap<Long, TnccsSerializer<AbstractPbMessageValue>>());
			pbMessageTranslators.get(vendorId).put(messageType, validator);
		}
	...
	public void removeSerializer(Long vendorId, Long messageType) {
		if(pbMessageTranslators.containsKey(vendorId)){
			...
			if(pbMessageTranslators.get(vendorId).isEmpty()){
				pbMessageTranslators.remove(vendorId);
			}
		}
	}
}
