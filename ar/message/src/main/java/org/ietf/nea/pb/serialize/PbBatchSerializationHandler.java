...
public class PbBatchSerializationHandler implements
		TnccsSerializationHandler<PbBatch> {

	private static final int BATCH_HEAD_SIZE = PbBatch.FIXED_LENGTH; /* in byte */

	TnccsSerializationHandler<PbMessage> messageHandler;
	
	public PbBatchSerializationHandler(TnccsSerializationHandler<PbMessage> messageHandler) {
	...
	@Override
	public PbBatch decode(InputStream in, long length) throws SerializationException {
	...
	private long createBatchHeader(final PbBatchBuilderIetf builder, final byte[] data) throws SerializationException{
		...
		byte directionality = (byte) (data[0] >>> 7);
