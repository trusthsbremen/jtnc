...
public class PbMessageAccessRecommendationSerializationHandler implements TnccsSerializationHandler<PbMessageValueAccessRecommendation> {

	private static final int MESSAGE_VALUE_FIXED_SIZE = PbMessageValueAccessRecommendation.FIXED_LENGTH;
	
	private static final class Singleton{
		private static final PbMessageAccessRecommendationSerializationHandler INSTANCE = new  PbMessageAccessRecommendationSerializationHandler();
	}
	  
	
	public static  PbMessageAccessRecommendationSerializationHandler getInstance(){
	    	return Singleton.INSTANCE;
	}
	    
	private  PbMessageAccessRecommendationSerializationHandler(){
	    	// Singleton
	}
	...
	public PbMessageValueAccessRecommendation decode(final InputStream in, final long length) throws SerializationException {
		...
		if (count >= MESSAGE_VALUE_FIXED_SIZE){
			...
			if(recommendation != null){
				value = new PbMessageValueAccessRecommendation((short)0, recommendation);
			}else{
