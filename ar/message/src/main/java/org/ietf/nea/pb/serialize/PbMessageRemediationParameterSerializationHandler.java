...
import org.ietf.nea.pb.message.AbstractPbMessageValueRemediationParametersValue;
...
public class PbMessageRemediationParameterSerializationHandler implements TnccsSerializationHandler<PbMessageValueRemediationParameters> {

	private static final int MESSAGE_VALUE_FIXED_SIZE = PbMessageValueRemediationParameters.FIXED_LENGTH;
	
	private static final class Singleton{
		static final PbMessageRemediationParameterSerializationHandler INSTANCE = new  PbMessageRemediationParameterSerializationHandler();  
	}
	
	private final PbMessageRemediationParameterStringSerializationHandler stringParamsSerializer;
	
	private final PbMessageRemediationParameterUriSerializationHandler uriParamsSerializer;

	public static  PbMessageRemediationParameterSerializationHandler getInstance(){
	    	return Singleton.INSTANCE;
	}
 
    private PbMessageRemediationParameterSerializationHandler() {
    	// Singleton
    	this.stringParamsSerializer = PbMessageRemediationParameterStringSerializationHandler.getInstance();
    	this.uriParamsSerializer = PbMessageRemediationParameterUriSerializationHandler.getInstance();
    }	
	...
	public PbMessageValueRemediationParameters decode(final InputStream in, final long length) throws SerializationException {
		...
		if (count >= MESSAGE_VALUE_FIXED_SIZE){
			...
			if(rpType != null){
				rpParam = decodeTypedPbMessageRemediationParameter(rpType, in);
			}else{
				throw new SerializationException("Remediation type #"+type+" could not be recognized.", Long.toString(type));
			}
			
			value = new PbMessageValueRemediationParameters(reserved, rpVendorId, rpType.type(), rpParam);
