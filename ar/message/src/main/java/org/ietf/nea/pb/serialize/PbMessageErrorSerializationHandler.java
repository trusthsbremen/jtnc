...
public class PbMessageErrorSerializationHandler implements TnccsSerializationHandler<PbMessageValueError> {

	private static final int MESSAGE_VALUE_FIXED_SIZE = PbMessageValueError.FIXED_LENGTH;
	private static final class Singleton{
		private static final  PbMessageErrorSerializationHandler INSTANCE = new  PbMessageErrorSerializationHandler();  
	}

	public static  PbMessageErrorSerializationHandler getInstance(){
	    	return Singleton.INSTANCE;
	}
	    
	private  PbMessageErrorSerializationHandler(){
	    	// Singleton
	}
	...
	public void encode(final PbMessageValueError data, final OutputStream out)
		...
		/* Error Code */
		byte[] code = ByteBuffer.allocate(2).putShort(data.getErrorCode().code()).array();
		try {
			buffer.write(code);
		} catch (IOException e) {
			throw new SerializationException(
					"Error code could not be written to the buffer.", e,
					Short.toString(data.getErrorCode().code()));
		}
	...
	public PbMessageValueError decode(final InputStream in, final long length) throws SerializationException {
		...
		if (count >= MESSAGE_VALUE_FIXED_SIZE){
			...
			/* Error Code */
			short code = ByteArrayHelper.toShort(new byte[]{buffer[4], buffer[5]});
			// TODO it may be better to check the relation between error vendor ID and error code here.
			PbMessageErrorCodeEnum errorCode = PbMessageErrorCodeEnum.fromCode(code);

			if(errorCode == null){
				throw new SerializationException("Error code #"+code+" could not be recognized.", Short.toString(code));
			}
			...
			value = new PbMessageValueError(errFlags, errorVendorId, errorCode, reserved, errorParameter);
