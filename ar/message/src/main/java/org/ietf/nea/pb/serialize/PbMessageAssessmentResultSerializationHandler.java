...
public class PbMessageAssessmentResultSerializationHandler implements TnccsSerializationHandler<PbMessageValueAssessmentResult> {

	private static final int MESSAGE_VALUE_FIXED_SIZE = PbMessageValueAssessmentResult.FIXED_LENGTH;
	private static final class Singleton{
		private static final PbMessageAssessmentResultSerializationHandler INSTANCE = new  PbMessageAssessmentResultSerializationHandler(); 
	}
	public static  PbMessageAssessmentResultSerializationHandler getInstance(){
	    	return Singleton.INSTANCE;
	}
	    
	private  PbMessageAssessmentResultSerializationHandler(){
	    	// Singleton
	}
	...
	public PbMessageValueAssessmentResult decode(final InputStream in, final long length) throws SerializationException {
		...
		if (count >= MESSAGE_VALUE_FIXED_SIZE){
			...
			if(result != null){
				value = new PbMessageValueAssessmentResult(result);
			}else{
