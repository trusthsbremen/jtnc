...
public class PbMessageImSerializationHandler implements TnccsSerializationHandler<PbMessageValueIm> {

	private static final int MESSAGE_VALUE_FIXED_SIZE = PbMessageValueIm.FIXED_LENGTH;
	
	private static final class Singleton{
		private static final PbMessageImSerializationHandler INSTANCE = new  PbMessageImSerializationHandler();  
	}
	
	public static  PbMessageImSerializationHandler getInstance(){
	    	return Singleton.INSTANCE;
	}
	    
	private  PbMessageImSerializationHandler(){
	    	// Singleton
	}
	...
	public PbMessageValueIm decode(final InputStream in, final long length) throws SerializationException {
		...
		if (count >= MESSAGE_VALUE_FIXED_SIZE){
			...
			value = new PbMessageValueIm(imFlags, subVendorId, subType, collectorId, validatorId, imMessage);
